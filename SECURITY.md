...
| `https://*.google-analytics.com`, `https://*.google.com` | `connect-src` | gtag's measurement requests, exactly as Google's GA4 guidance for the Google tag lists them (developers.google.com/tag-platform/security/guides/csp, read 2026-09-26; the site uses no Ads features, which need more). The browser check saw `www.google-analytics.com`. `*.analytics.google.com`, in 0.2.31, is gone: the guidance does not list it and no traffic used it |
...
**The guard.** `npm run check:csp` runs after the build in `check-posts.yml` and both deploy workflows, before any upload (a test fails if a workflow that builds the site does not run it). It fails when a built page has an inline script missing from the policy, an inline event handler (`onclick=…`), a `javascript:` URL, an `<object>`, `<embed>` or `<base>`, an iframe `srcdoc` (its document inherits the page's policy, and the guard does not look inside it), or a load the policy does not allow. The loads it checks, each against its directive (or `default-src`): scripts (`<script src>`, and an SVG `<script href>` or `xlink:href`, both external and never hashed), iframes, form actions (also against `connect-src`, since the forms `fetch` them), images (`<img src>` and `srcset`, `<picture>` sources, video posters, `<input type=image>`; so an `http:` image fails), media (`<video>`, `<audio>`, `<source>`, `<track>`), `<link>` stylesheets, preloads (by `as`), module preloads, icons and manifests, and the site's own `data-endpoint`/`data-embed` attributes. It also fails when a `_headers` rule is a form it does not model (only exact paths and a trailing `*`; a `:placeholder`, a `*` mid-path or an absolute URL is refused rather than misjudged), when a path would get two policies, when Pagefind's worker would lack `'wasm-unsafe-eval'`, when a header line exceeds Cloudflare's 2,000 characters, or when `dist/_headers` is not exactly `public/_headers` plus the policy this build calls for. Event handlers and `javascript:` URLs fail the build itself too: use `addEventListener` in a script.

**Endpoints.** The comments and contact origins follow `PUBLIC_COMMENTS_ENDPOINT` and `PUBLIC_CONTACT_ENDPOINT` exactly as the build reads them: Vite's `loadEnv` for the production mode (`.env`, `.env.local`, `.env.production`, `.env.production.local`, with the process's environment first), so a local build pointed at `wrangler dev` gets a matching policy. A script type is judged by its MIME essence (the part before any `;`): a script that some browser might run is hashed, and a hash too many costs nothing.
