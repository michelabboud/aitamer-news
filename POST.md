...
- Slugs are lowercase words joined by `-`, at most 120 characters (the desk's comments Worker refuses a longer one, so `npm run check:posts` does too). Two posts cannot share a slug, since they would be the same file. When a name is taken, be more specific (`grok-4-7-pricing`).
...
**The desk's publisher writes these files, and nobody edits them by hand.** The publisher regenerates each file from the desk's database on every run, so a file is a copy, not the record: a comment deleted from a file by hand comes back on the next publish.

**Removing a comment** is done with the desk's delete command, part of the desk's moderation tool. It takes the comment out of the desk's database, erases the stored name and text there, and the next publish rewrites the post's file without it (or deletes the file, when it was the last comment); the comment leaves the site at the deploy that follows. A published comment stays in this repository's public git history either way, which no removal can erase; the privacy page says so. **A hand edit is an emergency stopgap only** — for a comment that must come off the site before the desk's next publish: delete its entry (or the file), commit it to `main`, and run the desk's delete for the same comment id the same day, or the next publish puts the comment back.
...
- `version` is `1`; `slug` is the post's slug and the file's name (at most 120 characters); `generatedAt` and every `at` are UTC times ending in `Z`, and no `at` is later than `generatedAt`.
- `comments` holds at least one and at most 2,000 comments, oldest first. A post with no approved comments has **no file**.
- `id` is a ULID (26 uppercase Crockford base32 characters, the first `0`–`7`), unique in the file. `name` is 1–60 characters on one line. `text` is 1–2,000 characters of plain text: paragraphs separated by a blank line, `\n` the only control character. Both need at least one visible character and refuse invisible, format, private-use and bidirectional characters (one joiner, U+200C or U+200D, only between two other characters, never two in a row) and HTML (`<` may not be followed by a letter, `/`, `!` or `?`). Links are plain `https://…` text. Lengths count Unicode code points. `signedIn` is an optional boolean.
...
**How the publisher gets its files in.** It never pushes to `main`: it pushes a `desk/comments-*` branch and opens a pull request, and the required check `publisher-paths` (`.github/workflows/check-publisher-pr.yml`, `scripts/check-publisher-paths.mjs`) refuses the pull request if any changed path is not `src/content/comments/<slug>.json` or, for reaction totals (section 9), `src/content/reactions/<slug>.json`, if a file is renamed from or to anywhere else (a rename between those two directories included, when git detects it as a rename; a dissimilar delete-and-add is two allowed changes, and each file's contract is what holds it to its directory's format), or if an added or changed file is a symlink, a submodule or executable. Deleting a comment file is allowed: that is how an emptied thread leaves the site. The rule applies to every pull request unless both its author and the account that triggered the run are the maintainer (repository variable `MAINTAINER_ID`, compared by numeric account id; unset means every pull request is held to it), so the publisher cannot slip changes into someone else's pull request (`CONTRIBUTING.md`). As a second line, the deploy refuses a push to `main` made by the publisher (repository variable `PUBLISHER_ACTOR_ID`, the App's numeric account id) that touches anything outside those two directories. That second line judges one push only: a refused commit stays on `main` and would ship with the next deploy, so the first thing to do after a refusal is revert it (`docs/adr/0007-the-publisher-guard-as-built.md`). Until it is reverted, disable the `Scheduled publish` workflow too (`gh workflow disable scheduled-publish.yml`): its hourly run starts a deploy without any push, which the guard does not judge, and would ship the refused commit.

**Closing a thread.** Set `comments: { closed: true }` in the post's frontmatter (section 2). The page then shows "Comments are closed on this story." instead of the form, and the build writes the thread as `closed` in `/comments/threads.json`, which the desk's Worker reads to refuse a new comment for that slug. Published comments stay on the page; take one down with the desk's delete command (above), never by editing the data file alone. A withdrawn post is closed without the field.
...
Enforcement is the comments' own: `npm run check:posts` (`scripts/check-reactions.mjs`) fails a file whose post does not exist, whose `slug` is not its file name, or that is larger than 8 KiB (far above the largest valid file), and any entry in `src/content/reactions/` that is not `README.md` or a regular `<slug>.json` file; `npm run build` fails a file that breaks the format (`src/content/reaction-schema.ts`, strict at every level), naming it. The same schema is published as JSON Schema at `/contract/reactions.schema.json` (newest) and `/contract/v1/reactions.schema.json` (v1, frozen) for the desk to validate against before it commits. On the page (once live): a React button under the article and, beside it, the top three reactions and the total; a story whose comments are closed (`comments: { closed: true }`) keeps its totals but takes no new reaction. Detail: `src/content/reactions/README.md`. The publisher lands these files the way it lands comment files, in the same pull request, and the same guard holds them to their directory (section 8, "How the publisher gets its files in"; ADR 0008).
