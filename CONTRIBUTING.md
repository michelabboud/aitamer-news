...
**Second line, in the deploy.** `deploy-pages.yml` runs the same script in `push` mode as its first step after checkout, before `npm ci`: when `github.actor_id` (the account that pushed) is the repository variable **`PUBLISHER_ACTOR_ID`** (the App's numeric bot account id, `gh api 'users/<app-slug>[bot]' --jq .id`), every path changed between the push's `before` and `after` must be a comment file, or nothing is built or deployed. `PUBLISHER_ACTOR_ID` unset or blank: the step logs a notice and skips; set but not a numeric id: the deploy fails. A `before` of all zeros or one that cannot be fetched fails, and so does a non-push run started by the publisher. The pushed tree is the thing being judged, so the step takes the script (and `scripts/slug.mjs`) from the commit before the push with `git show`, and decides who pushed in the workflow's own shell (tested as it stands by `scripts/deploy-publisher-guard.test.mjs`). It is only a second line: the workflow file itself comes from the pushed commit (the App cannot change workflow files without the `workflows` permission).
