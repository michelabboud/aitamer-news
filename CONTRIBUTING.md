...
The rule (phase 2 plan D12, widened by ADR 0008): every changed path must be `src/content/comments/<slug>.json` or `src/content/reactions/<slug>.json` — the publisher's two lanes; not nested, not another name, extension or case, nothing outside those two directories; a rename that git detects must start and end inside **the same** lane — a tripwire, not a guarantee: git pairs a deletion and an addition as a rename only when they are at least half alike, so a deletion in one lane plus a dissimilar addition in the other passes as two lane changes, and it is each lane's contract (the required check `check`) that keeps a file in the shape its directory needs (ADR 0008, amendment); an added or changed file must be a plain, non-executable file (no symlink, no submodule, no executable bit). Deleting a file in a lane is allowed: that is how an emptied thread, or a story whose reactions are back to none, leaves the site. Anything that cannot be judged (a missing commit, unreadable git output) fails.
